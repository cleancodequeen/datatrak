...
class Mobile_Menu extends Walker_Nav_Menu {
	function start_el(&$output, $item, $depth, $args) {
		...
		if ($depth == 0) {
			$item_output .= $indent . '<div class="primary-button">';
		}
		...
		if ($depth == 0) {
			$item_output .= $indent . '<div class="expand">&#9776;</div>';
			$item_output .= '</div>';
		}
...
class Footer_Site_Map extends Walker_Nav_Menu {
	function start_el(&$output, $item, $depth, $args) {
		global $wp_query;
		$indent = ( $depth ) ? str_repeat( "\t", $depth ) : '';
